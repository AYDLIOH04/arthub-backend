...
export class BrushService {
    ...
    async createBrush(dto: BrushDto, image: any){
        ...
        const newBrush = await this.prisma.brush.create({data: {...dto, image: fileName}});
        return newBrush
    }
    ...
    async addToUser(brushID, userID ) {
        ...
        else {throw new HttpException('Такой уже есть', HttpStatus.INTERNAL_SERVER_ERROR)}

    }

    async removeFromUser(brushID, userID ) {
        const user = await this.prisma.user.findUnique({where: {id: Number(userID)}})
        const updatedBrushes = user.brushes.filter((brush) => brush != Number(brushID))
        await this.prisma.user.update({where: {id: Number(userID)}, data: {brushes: updatedBrushes}})
        return user
    }
