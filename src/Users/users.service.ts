...
export class UsersService {
    ...
    async deleteUser(userId) {
        let user = await this.prisma.user.delete({where: {id: parseInt(userId)}});
        this.checkUser(user)
        return user
    }
    ...
    async sortBrushByProgramm(program, userID) {
        ...
        if (flag) {
            ...
            for (let id of user.brushes) {
                userBrushes.push(await this.prisma.brush.findUnique({where: {id: id}}))
            }
    ...
    async sortReferenceByHashtag(hashtag, userID) {
        ...
        for (let id of user.references) {
            userReferences.push(await this.prisma.reference.findUnique({where: {id: id}}))
        }
