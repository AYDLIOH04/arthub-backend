...
export class ReferenceController {
    ...
    addToUser(@Param('referenceID') referenceID: string, @Param('userID') userID: string) {
        return this.referenceService.addToUser(referenceID, userID)
    }

    @Public()
    @Post('remove/:referenceID/:userID')
    removeFromUser(@Param('referenceID') referenceID: string, @Param('userID') userID: string) {
